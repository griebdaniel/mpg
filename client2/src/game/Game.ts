...
export default class Game extends Scene {
  ...
  private totalLifeCount = 3;

  onLifeOut = new Subject<void>();
  onTargetMiss = new Subject<Vector3>();
  score = 0;
  stackCounter = 0;
  ...
  update(time: number): void {
    ...
    const inseparableCircles = [];
    try {
      const inseparableCircles = this.separateCircles(targets);
    } catch (e) { 
      inseparableCircles.forEach(target => target.restoreState());
    }

    targets.forEach(target => {
      target.saveState();
      target.updateSize(time);
      if (target.sizeChangeValue > 1) {
        target.visible = false;
        this.onTargetMiss.next(target.position);
      }
    });
    try {
      this.separateCircles(targets);
    } catch (e) { 
      targets.forEach(target => target.restoreState());
    }
    ...
    for (let i = 0; i < circles.length; i++) {
      ...
      for (let j = i + 1; j < circles.length; j++) {  
        ...
        if (circle.size + circle2.size - circle.position.distanceTo(circle2.position) > 0) {
          ...
          circle.direction = d1.normalize();
          circle2.direction = d2.normalize();
        }
      }
    }
  }

  separateCircles(circles: Target[]): Target[] {
    ...
    if (changedCircles.length > 0) {
      if (this.stackCounter++ > 100) {
        this.stackCounter = 0;
        return changedCircles;
      }
      return this.separateCircles(circles);
    }
  }

  reset(): void {
    this.targets.forEach(target => target.visible = false);
    this.clicks.forEach(click => click.visible = false);
