...
export class TestTarget extends Group {
  ...
  private previousPosition = new Vector3(0, 0, 0);
  ...
  updateSize(time: number): void {
    if (this.sizeChangeDistribution === 'Linear') {
      ...
      this.size = this.sizeChangeFunction(this.sizeChangeValue) * (this.maxSize - this.minSize) + this.minSize;
  ...
  set speed(speed: number) {
    ...
    // scaling the circle scales the arrow too. to ignore circle scale you need `speed / this.size`
